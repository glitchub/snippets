...
class ProgressBar
{
    ...
    std::string append;     // string to write after bar reaches 100%

    public:
    ProgressBar(int _expected)
    {
    ...
    void Draw()
    {
        if (fancy)
        {
            std::cout << '[' << std::string(current/2, '#') << std::string(50 - current/2, ' ') << "] " << current << '%';
            if (current == 100) std::cout << append;
        }
        else
        {
            std::cout << '[' << std::string(current/2, '#');
