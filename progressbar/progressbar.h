...
//       ProgressBar bar(expected, "Label: ");
...
//   }
//
// The constructor is given the expected number of events, and an optional label.  Update() is then
// called for each event in order to progressively update the bar.
//
// The caller should not write to stdout until all updates have been performed. If for some reason
// the cursor must moved between Update()'s then Draw() can called to recreate the bar (with label)
// in its current state at the new cursor position. Draw() can also be called before the first
// Update() to show a preliminary "empty" bar.
//
// If stdout is a tty then the "fancy" progress bar is enabled, this uses spaces and backspaces to
// position the cursor when updating the bar. Examples at 0, 50 and 100%:
//
//    Label: [                                                  ] 0%
//    Label: [#########################                         ] 50%
//    Label: [##################################################] 100%
...
// bar until it reaches 100%, therefore does not require cursor positioning. Examples at 0, 50 and
// 100%:
//
//    Label: [
//    Label: [#########################
